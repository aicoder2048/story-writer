---
description: 自动创作一部完整的短篇小说（一键生成，支持武侠/科幻/童话/现言类型）
argument-hint: <类型> <故事名> <story_spec文件路径>
...
[参数]
    ...
    $2: 故事名（用于创建项目目录和章节文件命名）
    $3: story_spec文件路径（包含Q1核心创意、Q2核心冲突、Q3故事调性，以及可选的扩展设定和素材）
...
[自动化工作流程]
    ...
    [第一阶段：初始化与参数解析]
        ...
        4. 显示启动信息（使用从genre-config.md读取的ASCII艺术和启动语）：
           "[ASCII艺术]
    ...
    [第十二阶段：完成报告]
        ...
        2. 计算总耗时
        3. 统计各章节字数
        4. 读取 genre-config.md 获取完成语
        5. 输出完成报告：

        "**短篇小说《`STORY_NAME`》创作完成！**
