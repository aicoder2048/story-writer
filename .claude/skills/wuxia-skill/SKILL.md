---
name: wuxia-skill
description: 古风武侠短篇小说创作技能包。执行大纲、人物、目录、章节的专业创作，基于金庸武侠创作法则和典雅古风的写作风格生成高质量小说内容。
---

# 武侠小说创作 Skill

[技能说明]
    专业的古风武侠短篇小说创作技能包，覆盖故事构思、人物塑造、章节规划、正文写作全流程。根据不同创作阶段，读取对应的创作资源并生成符合金庸武侠规范的小说内容。

[文件结构]
    .claude/skills/wuxia-skill/
...
[核心能力]
    ...
    - **专业创作**：基于资源和上下文创作符合武侠规范的小说内容
    - **风格把控**：确保创作内容符合金庸武侠的典雅古风写作风格
    - **模板遵循**：严格按照模板格式生成文档结构
...
[执行流程]
    ...
    第二步：读取创作资源
        **大纲创作阶段**：
            ...
            2. 读取 Outline-method.md（武侠大纲创作方法论，含人物塑造指导）
            3. 读取 Output-style.md（武侠写作风格）
            4. 读取 examples/outline-example.md（武侠大纲示例）
            5. 从对话历史获取用户回答的Q1-Q3（核心创意、核心冲突、故事调性）

        **人物创作阶段**：
            1. 读取 outline.md（了解故事背景和设定）
            2. 读取 templates/character-template.md（文档格式模板）
            3. 读取 Outline-method.md（武侠人物塑造指导部分）
            4. 读取 Output-style.md（武侠写作风格）
            5. 读取 examples/character-example.md（武侠人物示例）

        **目录创作阶段**：
            1. 读取 outline.md 和 character.md（了解故事和人物）
            2. 读取 templates/chapter-index-template.md（文档格式模板）
            3. 读取 Outline-method.md（武侠章节规划方法）
            4. 读取 Output-style.md（武侠写作风格）

        **章节创作阶段**：
            1. 读取 outline.md、character.md、chapter_index.md（全部上下文）
            2. 读取 templates/chapter-template.md（文档格式模板）
            3. 读取 Output-style.md（武侠写作风格和章节创作方法，最重要）
            4. 读取 examples/chapter-example.md（武侠章节示例）
    ...
    第三步：执行专业创作
        **大纲创作**：
            ...
            - 遵循 Outline-method.md 的武侠创作方法论
            - 应用 Output-style.md 的典雅古风写作风格
            - 参考 examples/outline-example.md 的示例
            - 生成包含以下内容的完整大纲：
                • 小说信息（书名、类型：武侠、故事调性）
                • 核心梗概（一句话）
                • 导语（150-200字，包含江湖元素和情感钩子）
        ...
        **人物创作**：
            ...
            - 遵循 Outline-method.md 中的武侠人物塑造指导
            - 应用 Output-style.md 的典雅古风写作风格
            - 参考 examples/character-example.md 的示例
            - 生成包含以下内容的人物小传：
                • 主要角色（3-5个，详细描述，强调武功路数和江湖身份）
                • 反派角色（如有）
                • 重要配角（如有）
                • 其他角色（如有）
            - 确保人物形象鲜明，具有武侠特质
        ...
        **目录创作**：
            ...
            - 遵循 Outline-method.md 的武侠章节规划方法
            - 应用 Output-style.md 的写作风格
            - 生成固定5章的章节目录，与大纲的起承转合结构对应
            - 每章包含：章节标题 + 一句话剧情简介（体现江湖气息和侠义精神）
            - 确保起承转合布局合理，故事节奏流畅

        **章节创作**：
            基于全部上下文文档和读取的资源：
            - 严格按照 templates/chapter-template.md 的格式结构
            - 严格遵循 Output-style.md 的典雅古风写作风格和章节创作方法（最重要）
...
[创作原则]
    ...
    - **质量标准原则**：
        • 大纲：江湖元素丰富，主题彰显侠义，故事结构完整，固定起承转合结构，2000-3000字
        • 人物：形象鲜明，具有武侠特质，武功与性格匹配
        • 目录：固定5章，与大纲的起承转合结构对应，布局合理，节奏流畅
        • 章节：2000-3000字，风格统一，典雅古风，推进剧情

[注意事项]
    - 确保每个阶段的必需资源都已读取完整
    - 不仅读取文档内容，还要深入理解其含义
    - Output-style.md中的典雅古风风格要求是强约束，必须严格遵守
    - templates/中的格式是必须遵循的，任何偏离都可能导致问题
    - 短篇小说固定为5章结构，采用起承转合布局
    - 创作内容必须完整、连贯、符合武侠特点
    - 始终使用中文创作
    - 武侠世界观需符合金庸风格：江湖门派、武功招式、侠义精神
